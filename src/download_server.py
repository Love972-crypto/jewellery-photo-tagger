...
class DownloadServerInfo:
    base_url: str
    runtime_root: Path
    _server: ThreadingHTTPServer = field(repr=False, compare=False)
...
def start_download_server(project_root: Path, preferred_port: int | None = None) -> DownloadServerInfo:
    runtime_root = (project_root / ".runtime").resolve()
    runtime_root.mkdir(parents=True, exist_ok=True)

    handler = _build_handler(runtime_root)
    last_error: OSError | None = None
    candidate_ports = (0,) if preferred_port is None else range(preferred_port, preferred_port + 20)
    for port in candidate_ports:
        try:
            ...
            thread = threading.Thread(
                ...
                name=f"sunaar-download-{actual_port}",
                daemon=True,
            )
            thread.start()
